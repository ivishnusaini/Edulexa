...
class ChatMessageActivity : AppCompatActivity(), View.OnClickListener {
    ...
    var binding: ActivityChatMessageStudentBinding? = null
    var chatIdUpdate = ""
    var chatIdNewMessage = ""
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityChatMessageStudentBinding.inflate(layoutInflater)
    ...
    private fun getBundleData() {
        try {
            ...
            chatIdUpdate = bundle.getString(Constants.StudentChat.CHAT_ID_UPDATE)!!
            chatIdNewMessage = bundle.getString(Constants.StudentChat.CHAT_ID_NEW_MESSAGE)!!
            chatConnectionId = bundle.getString(Constants.StudentChat.CHAT_CONNECTION_ID)!!
    ...
    private fun setUpData() {
        (Objects.requireNonNull<ItemAnimator>(binding!!.chatMessageRecycler.getItemAnimator()) as SimpleItemAnimator).supportsChangeAnimations =
            false
    ...
    private fun getChatRecord() {
        if (Utils.isNetworkAvailable(mActivity!!)) {
            ...
            jsonObject.put(Constants.ParamsStudent.CHAT_CONNECTION_ID, chatConnectionId)
            ...
            Utils.printLog("Url", Constants.DOMAIN_STUDENT + "/Webservice/getChatRecord")

            val call: Call<ResponseBody> = apiInterfaceWithHeader.getChatRecord(requestBody)
                        ...
                        if (!responseStr.isNullOrEmpty()) {
                            ...
                            if (status) {
                                val modelResponse = Utils.getObject(
                                    responseStr,
                                    ChatRecordResponse::class.java
                                ) as ChatRecordResponse
                                if (modelResponse.getData() != null) {
                                    if (modelResponse.getData()!!
                                        ...
                                        for (model in modelResponse.getData()!!.getChatList()!!) {
                                            ...
                                            chatMessageModel.setChatUserId(model!!.getChatUserId()!!)
                                        ...
                                        binding!!.chatMessageRecycler.scrollToPosition(chatMessageAdapter!!.getItemCount()-1)
                            ...
                            if (status) {
                                ...
                                if (modelResponse.getData() != null) {
                                    if (modelResponse.getData()!!
                                        ...
                                        (messageList as ArrayList<ChatMessageModel>).clear()
                                        val size: Int =
                                            modelResponse.getData()!!.getChatList()!!.size - 1
                                        for (i in size - 1 downTo 0) {
                                            val chatRecord =
                                                modelResponse.getData()!!.getChatList()!!.get(i)
                                            val chatMessageModel = ChatMessageModel()
                                            chatMessageModel.setChatUserId(chatRecord!!.getChatUserId()!!)
                                            chatMessageModel.setIsFirst(chatRecord.getIsFirst()!!)
                                            chatMessageModel.setLastId(chatRecord.getId()!!)
                                            chatMessageModel.setMessage(chatRecord.getMessage()!!)
                                            chatMessageModel.setTime(chatRecord.getTime()!!)
                                            chatMessageModel.setStatus("save")
                                            messageList.add(chatMessageModel)
                                        }
                                        val objects: List<Any> = ArrayList()
    ...
    private fun getFormattedListPagination(objects: List<Any>) {
        if (objects.size > 0) {
            val date = getDate((objects.get(0) as ChatMessageModel).getTime())
            (messageObjectList as ArrayList<Any>).add(0, objects.get(0))
            if (objects.size > 1)
            ...
            for (i in 1..objects.size-1) {
                ...
                val date2 = getDate((objects.get(i - 1) as ChatMessageModel).getTime())
                if (!date1.equals(date2)) {
                    messageObjectList.add(0, objects.get(i))
                    messageObjectList.add(
                        0,
                        ChatDateM(getDate((objects.get(0) as ChatMessageModel).getTime()))
                    )
                } else messageObjectList.add(0, objects.get(i))
            }
        }
    }
    ...
    private fun getInsertLastMessageDate(currentDate: String) {
        ...
        if (messageObjectList.size > 0) {
            ...
            if (getDate(currentDate).equals(date)) {
                if (objects.size > 0)
                    (messageObjectList as ArrayList<Any>).add(objects.get(0))
            } else {
    ...
    private fun showConnectedMessage(list: List<ChatRecord?>?) {
        var flag = false
        var message = ""
        for (model in list!!) {
    ...
    private fun getUpdateChatData() {
        if (Utils.isNetworkAvailable(mActivity!!)) {
            ...
            jsonObject.put(Constants.ParamsStudent.CHAT_CONNECTION_ID, chatConnectionId)
            jsonObject.put(Constants.ParamsStudent.CHAT_TO_USER, chatIdUpdate)
            ...
            Utils.printLog("Url", Constants.DOMAIN_STUDENT + "/Webservice/chatUpdate")

            val call: Call<ResponseBody> = apiInterfaceWithHeader.getChatUpdate(requestBody)
            call.enqueue(object : Callback<ResponseBody> {
                override fun onResponse(
                    call: Call<ResponseBody>,
                    response: Response<ResponseBody>
                                ...
                                if (modelResponse.getData() != null) {
                                    if (modelResponse.getData()!!.getUserNewChat() != null) {
                                        if (modelResponse.getData()!!.getUserNewChat()!!
                                                ...
                                                .getUserNewChat()!!.getUpdatedChat()!!.size > 0
                                        ) {
                                            for (model in modelResponse.getData()!!
                                                .getUserNewChat()!!.getUpdatedChat()!!) {
                                                (messageList as ArrayList<ChatMessageModel>).clear()
                                                val chatMessageModel = ChatMessageModel()
                                                chatMessageModel.setChatUserId(model!!.getChatUserId()!!)
                                                chatMessageModel.setIsFirst(model.getIsFirst()!!)
                                            ...
                                            if (chatMessageAdapter != null)
                                                chatMessageAdapter!!.notifyItemInserted(
    ...
    private fun sendMessage() {
        ...
        if (messageStr.isEmpty())
            ...
            if (Utils.isNetworkAvailable(mActivity!!)) {
                ...
                jsonObject.put(Constants.ParamsStudent.TIME, Utils.getCurrentDate())
                ...
                Utils.printLog("Url", Constants.DOMAIN_STUDENT + "/Webservice/newMessage")

                val call: Call<ResponseBody> = apiInterfaceWithHeader.sendNewMessage(requestBody)
                call.enqueue(object : Callback<ResponseBody> {
                    override fun onResponse(
                        call: Call<ResponseBody>,
                        response: Response<ResponseBody>
